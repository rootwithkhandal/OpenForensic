...
| **🔍 Parallel Data Recovery & File Carving** | Multi-threaded Rayon file carving (`.jpg`, `.png`, `.pdf`, `.zip`/Office, `.sqlite`, `.evtx`) across unallocated sectors and physical `.dd`/`.raw` dumps. Uses a **Batched Streaming Architecture (`batch_limit = 16`)** guaranteeing strict ~256 MB bounded memory usage across multi-terabyte disk images without Out-Of-Memory crashes. Includes interactive single-threaded vs multi-threaded Rayon CPU benchmark verification. |
