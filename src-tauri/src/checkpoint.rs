...
pub fn load_checkpoint(
    ...
    // Validate that source matches, completed_offset <= expected_total_bytes, and partial hash matches
        ...
        && chk.completed_offset <= expected_total_bytes
    {
        if !chk.sha256_partial_hex.is_empty() {
            if let Ok(mut file) = File::open(destination_path) {
                if let Ok(hash) = hash_partial_file(&mut file, chk.completed_offset) {
                    if !hash.eq_ignore_ascii_case(&chk.sha256_partial_hex) {
                        return None;
                    }
                } else {
                    return None;
