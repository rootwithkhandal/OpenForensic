...
pub async fn mount_disk_image(
    ...
    if !path.exists() {
        return Err(format!("Disk image file not found: {}", image_path));
    }
        ...
        if ext == "vhd" || ext == "vhdx" || ext == "iso" || ext == "img" {
            ...
            if let Ok(out) = imdisk_check {
                if out.status.success() {
                    ...
                    let im_cmd = format!("imdisk -a -f \"{}\" -m \"{}\" {}", image_path, mp, ro_flag);
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let output = Command::new("mount")
            .args(&[ro_flag, "-o", "loop", &image_path, &mp])
            .output();
    ...
    #[cfg(target_os = "macos")]
    {
        let _ = Command::new("hdiutil")
            .args(&["attach", "-readonly", "-nomount", &image_path])
            .output();
    ...
    if let Ok(mut list) = get_mounted_images().lock() {
        list.retain(|m| m.image_path != image_path && m.mount_point != mount_point);
...
pub async fn unmount_disk_image(image_path: String) -> Result<String, String> {
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(list) = get_mounted_images().lock() {
            if let Some(info) = list.iter().find(|m| m.image_path == image_path) {
    ...
    #[cfg(target_os = "macos")]
    {
        let _ = Command::new("hdiutil").args(&["detach", &image_path]).output();
    }

    if let Ok(mut list) = get_mounted_images().lock() {
        list.retain(|m| m.image_path != image_path && m.mount_point != image_path);
    }
