...
pub fn verify_audit_log_chain(app: &AppHandle) -> Result<AuditChainVerificationReport, String> {
    ...
    // Verify against external cryptographic seal to detect silent truncation/rollback
    let home = std::env::var("USERPROFILE").or_else(|_| std::env::var("HOME")).unwrap_or_else(|_| ".".to_string());
    let seal_file = std::path::PathBuf::from(home).join(".openforensic").join("audit_chain_seal.json");
    if seal_file.exists() && total_records > 0 {
        if let Ok(content) = std::fs::read_to_string(&seal_file) {
            if let Ok(val) = serde_json::from_str::<serde_json::Value>(&content) {
                let sealed_id = val["latest_id"].as_i64().unwrap_or(0);
                let sealed_hash = val["latest_entry_hash"].as_str().unwrap_or("");
                let sealed_mac = val["seal"].as_str().unwrap_or("");
                let expected_mac = crate::hasher::generate_report_seal(&format!("{}:{}", sealed_id, sealed_hash), "AUDIT_SEAL");
                if sealed_mac == expected_mac {
                    if expected_prev != sealed_hash {
                        return Ok(AuditChainVerificationReport {
                            is_valid: false,
                            total_records,
                            broken_record_id: None,
                            message: format!("CHAIN TRUNCATION VIOLATION: Database rollback or record deletion detected! Secure seal reports latest hash {}, but database ended at {}", sealed_hash, expected_prev),
                        });
                    }
                }
            }
    ...
    Ok(AuditChainVerificationReport {
        ...
        total_records,
        broken_record_id: None,
        message: format!("CHAIN-OF-CUSTODY INTACT: All {} audit records cryptographically verified via SHA-256 hash chaining and tamper-evident seal.", total_records),
    })
}
...
pub fn log_audit_event(
    ...
    let last_id: i64 = conn.query_row("SELECT id FROM audit_logs ORDER BY id DESC LIMIT 1", [], |row| row.get(0)).unwrap_or(0);
    let seal_mac = crate::hasher::generate_report_seal(&format!("{}:{}", last_id, entry_hash), "AUDIT_SEAL");
    let home = std::env::var("USERPROFILE").or_else(|_| std::env::var("HOME")).unwrap_or_else(|_| ".".to_string());
    let seal_file = std::path::PathBuf::from(home).join(".openforensic").join("audit_chain_seal.json");
