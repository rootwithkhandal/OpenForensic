...
pub fn detect_encryption_from_bytes(header: &[u8]) -> EncryptionType {
    ...
    // Check F2FS superblock magic 0xF2F52010 at offset 1024 (or offset 0) and verify CP_FLAG_ENCRYPT feature bit
    if header.len() >= 1040 && header[1024..1028] == [0x10, 0x20, 0xf5, 0xf2] {
        let f2fs_feature = u32::from_le_bytes([header[1036], header[1037], header[1038], header[1039]]);
        if (f2fs_feature & 0x0001) != 0 { // CP_FLAG_ENCRYPT / F2FS_FEATURE_ENCRYPT
            return EncryptionType::AndroidFbe;
        }
    } else if header.len() >= 16 && header[0..4] == [0x10, 0x20, 0xf5, 0xf2] {
        let f2fs_feature = u32::from_le_bytes([header[12], header[13], header[14], header[15]]);
        if (f2fs_feature & 0x0001) != 0 {
            return EncryptionType::AndroidFbe;
        }
    }
