...
fn obfuscate_credential(secret: &str, is_cookie: bool) -> String {
    if secret.is_empty() {
        if is_cookie {
            return "[DPAPI/Keychain Encrypted Cookie Value]".to_string();
        } else {
            return "[Encrypted / Keychain Protected]".to_string();
        }
    }
    if secret.starts_with("v10") || secret.starts_with("v11") {
        return "[AES-GCM DPAPI Encrypted Keychain Blob]".to_string();
    }
    use sha2::{Digest, Sha256};
    let mut hasher = Sha256::new();
    hasher.update(secret.as_bytes());
    let hash = hex::encode(hasher.finalize());
    format!("[REDACTED FOR SECURITY - SHA-256: {}...]", &hash[..16])
}
        ...
        if let Ok(mut stmt) = db.prepare("SELECT COALESCE(host_key, ''), COALESCE(name, ''), COALESCE(value, ''), COALESCE(path, '/'), COALESCE(creation_utc, 0), COALESCE(expires_utc, 0), COALESCE(is_secure, 0) FROM cookies") {
            if let Ok(rows) = stmt.query_map([], |row| {
                ...
                for row in rows.flatten() {
                    ...
                    let val_str = obfuscate_credential(&value, true);
                    let _ = triage_db.execute(
        ...
        if let Ok(mut stmt) = db.prepare("SELECT COALESCE(origin_url, ''), COALESCE(username_value, ''), COALESCE(password_value, ''), COALESCE(date_created, 0) FROM logins") {
            if let Ok(rows) = stmt.query_map([], |row| {
                ...
                for row in rows.flatten() {
                    ...
                    let pass_display = obfuscate_credential(&password, false);
                    let status = if password.starts_with("v10") || password.starts_with("v11") {
                        "Windows DPAPI v10/v11 Encrypted".to_string()
                    } else if password.is_empty() {
                        "OS Keychain / DPAPI Encrypted".to_string()
                    } else {
                        "Cleartext Redacted (SHA-256 Logged)".to_string()
                    };
                    let _ = triage_db.execute(
